...
\caption{Kjønns- og aldersstandardiserte rater pr. 100 000 innbyggere, ryggkirurgi, 20 - 85 år, RHf’enes opptaksområder, 2012-\Sexpr{rappAar}. Gjennomsnitt i perioden (søyler) og enkeltår (punkter).}
\label{fig:AA_Ryggkirurgi_BoRHF1}
\end{figure}

\begin{figure}[ht]
\scalebox{0.9}{\includegraphics{Figurer/AA_Ryggkirurgi_BoHF1.pdf}}
\caption{Kjønns- og aldersstandardiserte rater pr. 100 000 innbyggere, ryggkirurgi, 20 - 85 år, helseforetakenes opptaksområder, 2012-\Sexpr{rappAar}. Gjennomsnitt i perioden (søyler) og enkeltår (punkter).}
...
\caption{Kjønns- og aldersstandardiserte rater pr. 100 000 innbyggere, nakkekirurgi, 20 - 85 år, RHf’enes opptaksområder, 2012-\Sexpr{rappAar}. Gjennomsnitt i perioden (søyler) og enkeltår (punkter).}
\label{fig:AA_Nakkekirurgi_BoRHF1}
\end{figure}

\begin{figure}[ht]
\scalebox{0.9}{\includegraphics{Figurer/AA_Nakkekirurgi_BoHF1.pdf}}
\caption{Kjønns- og aldersstandardiserte rater pr. 100 000 innbyggere, nakkekirurgi, 20 - 85 år, Hf’enes opptaksområder, 2012-\Sexpr{rappAar}. Gjennomsnitt i perioden (søyler) og enkeltår (punkter).}
\label{fig:AA_Nakkekirurgi_BoHF1}
\end{figure}

\begin{figure}[ht]
\scalebox{0.9}{\includegraphics{Figurer/Rygkirurgi_line1.pdf}}
\caption{Kjønns- og aldersstandardiserte rater pr. 100 000 innbyggere, ryggkirurgi, 20 - 85 år, RHf’enes opptaksområder, 2013-\Sexpr{rappAar} per år (punkter).}
\label{fig:Rygkirurgi_line1}
\end{figure}

\begin{figure}[ht]
\scalebox{0.9}{\includegraphics{Figurer/Nakkekirurgi_line1.pdf}}
\caption{Kjønns- og aldersstandardiserte rater pr. 100 000 innbyggere, nakkekirurgi, 20 - 85 år, RHf’enes opptaksområder, 2013-\Sexpr{rappAar} per år (punkter).}
...
colnames(tabAvdN)[dim(tabAvdN)[2] ]<- paste(min(RegData$OpAar),'-',rappAar)
...
registreringer gjort ved de respektive avdelinger siste 5 år. Vi ser at det er  
...
\Sexpr{datomax}. Siden oppstart i 2007 er det totalt registrert 38612 operasjoner i NKR degenerativ rygg. 
\par
...
rownames(tabAvdNnakke)[dim(tabAvdNnakke)[1] ]<- 'TOTALT, alle avdelinger:'
colnames(tabAvdNnakke)[dim(tabAvdNnakke)[2] ]<- paste(min(NakkeData$Aar),'-',rappAar)
...
Det er  
\Sexpr{dim(tabAvdNnakke)[1]-1} avdelinger som rapporterer til NKR degenerativ nakke. Totalt er det registrert \Sexpr{NtotNakke} operasjoner siden oppstart (2012). Av disse er \Sexpr{tabKjPstNakke[2]}\% utført på menn og \Sexpr{tabKjPstNakke[1]}\% på kvinner.
...
Lav utdanning er assosiert til dårligere operasjonsresultat. Andelen ryggopererte med høyere utdanning (høyskole eller universitet) var \Sexpr{rev(UtdanningAar)[1]} i \Sexpr{rappAar} mot  \Sexpr{UtdanningAar[1]} i \Sexpr{startAar}. 
...
%det aktuelle året. 
Reoperasjon gir generelt dårligere operasjonsresultat enn første gangs operasjon.
...
Undergrupper av pasientene kan imidlertid ha god effekt av reoperasjon, men det gjenstår å identifisere disse samt hvilken kirurgisk behandling som kan være mest aktuell.
...
Mange kirurger krever eller anbefaler røykeslutt før mer omfattende inngrep slik som fusjonskirurgi gjennomføres.
%\Sexpr{txt}
...
\Sexpr{BruktMikroAarPro[length(BruktMikroAarPro)]} \% i \Sexpr{rappAar} for prolapsoperasjoner. 
...
I \Sexpr{rappAar} hadde \Sexpr{AndelDegSponSS} \% av de som ble operert for spinal stenose  også en forskyvning mellom ryggvirvlene (Degenerativ spondylolistese). I internasjonal litteratur er det sprikende anbefalinger for om denne undergruppen skal ha tilleggsbehandling med avstivningsoperasjon (fusjonskirurgi).
 

Norske studier baser på data fra NKR har vist at tilleggseffekten er liten og assosiert til høyere kostnader (flere liggedøgn på sykehus). 
I 2018 blir det publisert en studie baser på data fra NKR, og tilsvarende registre i Sverige og Danmark (SWEspine og DANEspine). Denne viser stor praksisvariasjon mellom landene, men at tilleggbehandling med fusjonskirurgi ikke er assosiert til større behandlingseffektivitet, men økte kostnader. En pågående nasjonal RCT studie skal i samarbeid med NKR se nærmere på dette. Denne studien vil blant annet kunne svare på om undergrupper av pasienter med spinal stenose og degenerativ spondylolistese kan ha spesiell nytte av fusjonskirurgi.
\clearpage

Figur \ref{fig:degSponFusj} viser at det er stor variasjon i bruk av  fusjonskirurgi, 
for denne pasientgruppen, også mellom avdelinger på samme sykehus.
\begin{figure}[ht]
...
Nevrokirurgiske avdelinger bruker mindre fusjonskirurgi enn de ortopediske.
...
Det har vært en reduksjon i liggetid  på sykehus (ca 1 døgn) fram til \Sexpr{rappAar} for både lumbal prolaps og spinal stenose opererte. Figur \ref{fig:LiggetidAvdPro} og \ref{fig:LiggetidAvdSS} viser at det var stor variasjon i antall liggedøgn mellom sykehus og avdelinger i \Sexpr{rappAar}.
Dette kan henge sammen med økt bruk av mindre invasive operasjonsmetoder og mer dagkirurgi. 
Andelen operert med dagkirurgi for hhv lumbalt skiveprolaps og spinal stenose har gått fra 
...
<<Antibiotika, results='asis', echo=FALSE, eval=FALSE>>=
      Antibiotika <- RyggFigAndelerTid(RegData=RegData, outfile='Figurer/Antibiotika.pdf', enhetsUtvalg=enhetsUtvalg,
                                       variabel='Antibiotika', tittel=1)	#AndelTidLinjer
...
Sammenholdt med bakrunnsdata og virksomhetsdata kan resultatmålene imidlertid gi en pekepinn på hvor godt behandlingstilbudet fungerer på ulike sykehus. Indikasjonsstillingen («inngangsbilletten»)til kirurgi er imidlertid fortsatt det som i størst grad påvirker operasjonsresultatet: Fikk rett person, rett
...
Det er viktig å merke seg at pasienter som er operert i \Sexpr{rappAar-1} først får resultater fra ettårs oppfølgning i \Sexpr{rappAar}. 
...
operasjon for lumbalt prolaps i \Sexpr{rappAar-1}. Dette betyr at funksjonssvikten ble redusert 
...
% \end{itemize}
Andelen som opplever at de har blitt helt bra eller mye bedre ett år etter operasjon har ligget stabilt siden \Sexpr{startAar} og var 74\% for lumbalt prolaps og 60\% for spinal stenose opererte (\Sexpr{rappAar}) . Andelen  som angir at de er klart verre er henholdsvis har ligget stabilt rundt 3,0 \% for lumbalt prolaps og 5,5 \% spinal stenose opererte.
...
Svarert på dette spørsmålet vil avhenge av en rekke andre faktorer enn selve den kirurgiske behandlingen. Andelen pasienter operert for lumbalt prolaps som ett år etter behandlinga er fornøyde med behandlingen de fikk på sykehuset  
...
Lumbal spinal stenose opererte gjennomgående er litt mindre fornøyde behandlingen enn prolapspasienter.
...
operasjonstidspunktet var uendret fra 2011 til \Sexpr{rappAar} (47\%). 
...
dum <- NakkeFigAndelerGrVar(RegData=NakkeData, valgtVar='NDIendr12mnd30pst', datoTil = datoTil12mnd, 
...
Gjennomsnittsalder ved nakkeoperasjon var \Sexpr{AlderGjsnNakke} år i \Sexpr{rappAar} og \Sexpr{Elektivt} \% ble operert med elektiv, planlagt kirurgi. \Sexpr{Kvinner} \% var kvinner.  \Sexpr{ASA345} \% hadde ASA grad over II. Andelen eldre over 70 år som nakkeopereres har ligget jevnt rundt 5 \% frem til og med \Sexpr{rappAar}, men varierer noe mellom sykehus, spesielt mellom offentlige og private, Figur \ref{fig:NakkeAlder70Sh}.


\begin{figure}[ht]
\scalebox{0.7}{\includegraphics{Figurer/NakkeAlder70Sh.pdf}}
...
\begin{figure}[ht]
\scalebox{0.8}{\includegraphics{Figurer/NakkeKomplinfekSh.pdf}}
...
nakkekirurgi. Andelen som får dette har ligget stabilt og var 99,7 \% i \Sexpr{rappAar}.
...
''Har du blitt eller blir du behandlet i over 6 uker med antibiotika for dyp infeksjon i operasjonssåret?''  Forekomsten i \Sexpr{rappAar} var 
\Sexpr{AndelSaarinf} \% (totalt for bakre og fremre nakkekirurgi).  Andelen som har svart ja, ved hvert sykehus, på minst ett av disse spørsmålene er vist i Figur \ref{fig:NakkeKomplinfek3mndSh}.  
...
%\sbsubsection{Ønsket resultat («suksess»)}
