...
KvinnerNakke <- sprintf('%.0f' , (N1aar-sum(NakkeData1aar$ErMann))/Ntot1aarNakke*100)
...
\Sexpr{dim(tabAvdN)[1]-1} avdelinger som registrerer og at det i perioden \Sexpr{min(RegData$OpAar)} til \Sexpr{rappAar} totalt er registrert \Sexpr{NtotNakke} 
...
\Sexpr{datomax}. I peroden før 2010, det vil si fra og med 2007 til og med 2009 er det 
...
registreringer gjort ved de respektive avdelinger hvert år. Vi ser at det er  
\Sexpr{dim(tabAvdNnakke)[1]-1} avdelinger som registrerer og at det i perioden \Sexpr{min(NakkeData$Aar)} til \Sexpr{rappAar} totalt er registrert \Sexpr{Ntot} 
operasjoner. Av disse er \Sexpr{tabKjPstNakke[2]}\% utført på menn og \Sexpr{tabKjPstNakke[1]}\% på kvinner.
...
AlderFord <- RyggFigAndeler(RegData=RegData1aar, valgtVar='Alder', datoFra=datoFra1aar, datoTil=datoTil, enhetsUtvalg=0, outfile='Figurer/FigAlderFord.pdf', tittel=0)
...
Alder70Tid <- RyggFigAndelTid(RegData=RegData, datoFra = datoFra, valgtVar='alder70', outfile='Figurer/FigAlder70.pdf', tittel=0)

@ 

Gjennomsnittsalderen har økt jevnt fra \Sexpr{sprintf('%.1f',AlderAar['startAar'])} år 
i \Sexpr{startAar} til \Sexpr{sprintf('%.1f',AlderAar[as.character(rappAar)])} år i \Sexpr{rappAar}. 
...
all hovedsak håndterer denne pasientgruppen. I \Sexpr{rappAar} ble \Sexpr{Andel70} \% (\Sexpr{sum(RegData1aar$Alder>=70)} 
operasjoner) av alle
ryggoperasjonene meldt til NKR utført på personer over 70 år, mot \Sexpr{ Alder70Tid$AggVerdier$Hoved[1]} i \Sexpr{startAar} 
...
Andelen pasienter med fedme har vært jevt økende fra \Sexpr{sprintf('%.1f',AndelFedmeAar[startAar])} \% 
i \Sexpr{startAar}
...
FremmedSpraakAar <-  RyggFigAndelTid(RegData=RegData, valgtVar='Morsmal', aar = startAar:rappAar,
                                     outfile='Figurer/FigMorsmalAar.pdf', tittel=0)
...
UtdanningTid <- RyggFigAndelTid(RegData=RegData, valgtVar='Utd', aar = startAar:rappAar,
                                     outfile='Figurer/FigUtdAar.pdf', tittel=0)
...
UforTid <- RyggFigAndelTid(RegData=RegData, valgtVar='UforetrygdPre', outfile='Figurer/FigUforTid.pdf', tittel=0)
...
ErstatningTid <- RyggFigAndelTid(RegData=RegData, valgtVar='ErstatningPre', outfile='Figurer/FigErstatTid.pdf', tittel=0)
...
og \Sexpr{max(AndelTidlOp3SS)} \% for spinal stenosepasienter i perioden \Sexpr{startAar}-\Sexpr{rappAar}. 
...
RoykTid <- RyggFigAndelTid(RegData=RegData, valgtVar='Roker', outfile='Figurer/FigRokerTid.pdf', tittel=0)
...
%\Sexpr{txt}
Andel røykere har gått fra \Sexpr{RoykAar[1]} i \Sexpr{startAar} til \Sexpr{rev(RoykAar)[1]} i \Sexpr{rappAar}.
...
For spinal stenose er endringa fra \Sexpr{BruktMikroAarSS[1]} \% i \Sexpr{startAar} til 
...
De hyppigste tilstandene pasienter ble operert for i \Sexpr{rappAar} var prolapskirurgi (\Sexpr{round(HovedInngrep$Andeler[1,2])} \%) og spinal stenose (\Sexpr{AndelSSrappaar} \%). Tabell \ref{tab:AntHovedInngrep} viser fordeling av hovedinngrepstype, samt antall registrerte operasjoner for hver hovedinngrepstype.
...
Det har vært en klar reduksjon i liggetid  på sykehus (ca 1 døgn) fram til \Sexpr{rappAar} for både lumbal prolaps og spinal stenose opererte. Figur \ref{fig:LiggetidAvdPro} og \ref{fig:LiggetidAvdSS} viser at det er stor variasjon i antall liggedøgn mellom sykehus og avdelinger.
...
#datoTil=datoTil, enhetsUtvalg=enhetsUtvalg, outfile='Figurer/FigLiggetidFord.pdf', tittel=0)
...
dum <- RyggFigGjsnBox(RegData=RegData, outfile='Figurer/OswEndrPro.pdf', 
                      aar=startAar:(rappAar-1), valgtVar='OswEndr', hovedkat=1, ktr=ktr, tittel=0)
dum <- RyggFigGjsnBox(RegData=RegData, outfile='Figurer/OswEndrSS.pdf', 
                      aar=startAar:(rappAar-1), valgtVar='OswEndr', hovedkat=8, ktr=ktr, tittel=0)
...
operasjon for lumbalt prolapsopererte i \Sexpr{rappAar-1}. Dette betyr at funksjonssvikten ble redusert 
...
dum <- RyggFigAndelTid(RegData=RegData, outfile='Figurer/Osw20TidPro.pdf', 
                       aar=c(rappAar-2:rappAar-1), valgtVar='OswEndr20', hovedkat=1, ktr=ktr, tittel=0)
dum <- RyggFigAndelTid(RegData=RegData, outfile='Figurer/Osw30TidSS.pdf', 
                       aar=c(rappAar-2:rappAar-1), valgtVar='OswEndr30pst', hovedkat=8, ktr=ktr, tittel=0)

dum <- RyggFigGjsnBox(RegData=RegData, outfile='Figurer/OswEndrTidPro.pdf', 
                      aar=c(rappAar-2:rappAar-1), valgtVar='OswEndr', hovedkat=1, ktr=ktr, tittel=0)

dum <- RyggFigGjsnBox(RegData=RegData, outfile='Figurer/OswEndrTidSS.pdf', 
                      aar=c(rappAar-2:rappAar-1), valgtVar='OswEndr', hovedkat=8, ktr=ktr, tittel=0)
...
for pasienter operert i perioden \Sexpr{startAar}-\Sexpr{rappAar-1}. 
Tilsvarende ligger andel fornøyde spinal stenosepasienter mellom \Sexpr{round(min(as.numeric(FornoydSS)))} \% 
...
FornoydPro <- RyggFigAndelStabelTid(RegData=RegData, outfile='Figurer/FigFornoydPro.pdf',
      valgtVar='Fornoyd', hovedkat=1, aar=startAar:(rappAar-1), ktr=ktr, tittel=0)
FornoydSS <- RyggFigAndelStabelTid(RegData=RegData, outfile='Figurer/FigFornoydSS.pdf', 
      valgtVar='Fornoyd', hovedkat=8, aar=startAar:(rappAar-1), ktr=ktr, tittel=0)
...
\Sexpr{ODISSTidlOpAnt3['TRUE','FALSE']} \% til \Sexpr{ODISSTidlOpAnt3['TRUE','TRUE']} \%. 
...
operasjonstidspunkt av de som er registrert i NKR var uendret fra 2011 til \Sexpr{rappAar} (47\%). 
...
#Ngrense = 20,hovedkat = 1, opKat=1, tidlOp=4, outfile='Figurer/FigOsw48.pdf', tittel=0)
...
OswEndrLav <- RyggFigAndelerGrVar(RegData=RegData, aar=c((rappAar-2):(rappAar-1)), 
                                  ...
                                  outfile='Figurer/FigOswEndrLav.pdf', tittel=0)
Osw22Pro <- RyggFigAndelerGrVar(RegData=RegData, aar=c((rappAar-2):(rappAar-1)), 
                                valgtVar='Osw22',ktr=2, Ngrense = 20,
                                hovedkat = 1, opKat=1, tidlOp=4, 
                                outfile='Figurer/FigOsw22Pro.pdf', tittel=0)
Osw22SS <- RyggFigAndelerGrVar(RegData=RegData, aar=c((rappAar-2):(rappAar-1)), 
                               valgtVar='Osw22',ktr=2, Ngrense = 20,
                               hovedkat = 8, opKat=1, tidlOp=4, 
                               outfile='Figurer/FigOsw22SS.pdf', tittel=0)

OswEndr20 <- RyggFigAndelerGrVar(RegData=RegData, aar=aar=c((rappAar-2):(rappAar-1)), 
                                 valgtVar='OswEndr20',  ktr=2,Ngrense = 20,
                                 hovedkat = 1, opKat=1, tidlOp=4, 
                                 outfile='Figurer/FigOswEndr20.pdf', tittel=0)

OswEndr30pstSS <- RyggFigAndelerGrVar(RegData=RegData, aar=aar=c((rappAar-2):(rappAar-1)), 
                                      valgtVar='OswEndr30pst', ktr=2, Ngrense = 20,
                                      hovedkat = 8, tidlOp=4, 
                                      outfile='Figurer/FigOswEndr30pstSS.pdf', tittel=0)
@
...
KpInf3MndPro <- RyggFigAndelerGrVar(RegData=RegData, valgtVar='KpInf3Mnd', aar=(rappAar-1):rappAar, 
                                    Ngrense = 20,
                                    hovedkat = 1, outfile='Figurer/FigKpInf3MndPro.pdf', tittel=0)
KpInf3MndSS <- RyggFigAndelerGrVar(RegData=RegData, valgtVar='KpInf3Mnd', aar=(rappAar-1):rappAar, 
                                   Ngrense = 20,
                                   hovedkat = 8, outfile='Figurer/FigKpInf3MndSS.pdf', tittel=0)

KpInf3MndTidPro <- RyggFigAndelTid(RegData=RegData,  valgtVar='KpInf3Mnd', 
                                   hovedkat = 1, outfile='Figurer/FigKpInf3MndTidPro.pdf', tittel=0)
KpInf3MndTidPro <- RyggFigAndelTid(RegData=RegData,  valgtVar='KpInf3Mnd', 
                                   hovedkat = 8, outfile='Figurer/FigKpInf3MndTidSS.pdf', tittel=0)
@
For alle typer prolapsoperasjoner har
andel sårinfeksjoner (pasientrapportert) har blitt noe redusert fram til \Sexpr{startAar}, mens bruk av  forbyggende antibiotikabehandling har økte sterkt og i dag får nesten alle forebyggende antibiotikabehandling ved kirurgi. Siden \Sexpr{startAar} har andelen sårinfeksjoner ligget stabilt rundt 2 \% for prolapsopererte og rundt 3 \% for spinal stenose opererte.
...
<<NAKKE, echo=FALSE>>=
AlderGjsn <- round(mean(NakkeData1aar$Alder, na.rm = T))
...
variable <- c('alder70', 'OprIndikMyelopati')
for (var in variable) {
dum <- NakkeFigAndelerGrVar(RegData=NakkeData1aar, valgtVar=var, outfile=paste0('Figurer/Nakke', valgtVar,'Sh.pdf'))
}
...
dum <- NakkeFigAndelerGrVar(RegData=NakkeData, valgtVar='NDIendr12mnd', datoTil = datoTil12mnd, 
...
dum <- NakkeFigAndelerGrVar(RegData=NakkeData1aar, valgtVar='Stemme3mnd', fremBak = 1, myelopati = 0, 
                     outfile='Figurer/NakkeStemme3mndSh.pdf')

dum <- NakkeFigAndelerGrVar(RegData=NakkeData1aar, valgtVar='Svelg3mnd', fremBak = 1, myelopati = 0, 
                     outfile='Figurer/NakkeSvelg3mndSh.pdf')

@

\subsubsection{Alder, kjønn og komorbiditet}
Gjennomsnittsalder ved nakkeoperasjon var \Sexpr{AlderGjsn} år i \Sexpr{rappAar}. \Sexpr{Kvinner} \% var kvinner. \Sexpr{Elektivt} \% ble operert elektivt, planlagt kirurgi. Rundt  \Sexpr{ASA345} \% hadde ASA grad over II. Andelen eldre over 70 år som nakkeopereres har ligget jevnt rundt 5 \% frem til og med \Sexpr{rappAar}, men varierer noe mellom sykehus, spesielt mellom offentlige og private, Figur \ref{fig:NakkeAlder70Sh}.
...
dum <- NakkeFigAndelerGrVar(RegData=NakkeData, datoFra=datoFra2aar, datoTil=datoTil, 
                            valgtVar='Komplinfek3mnd', #fremBak = 2,
                     outfile='Figurer/NakkeKomplinfek3mndSh.pdf')

KomplinfekTid <- NakkeFigAndelTid(RegData=NakkeData, valgtVar='Komplinfek3mnd',
                     outfile='Figurer/NakkeInfekTid.pdf')
@
En av de hyppigste komplikasjonene etter nakkekirurgi er sårinfeksjon. Forekomsten i \Sexpr{rappAar} var 7 \% (totalt for bakre og fremre nakkekirurgi). 
\textit{LENA sjekk}, 
