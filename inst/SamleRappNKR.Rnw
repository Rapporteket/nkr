...
hovedkat <- 99
tidlOp <- 0	# Dvs. alle
#plotType <- 'S'	#S - søyle, L - linje Ikke i bruk
mT <- 1
ktr <- 2
...
#HKatdata <- HovedKatValg(RegData,hovedkat)
#RegDataLand <- HKatdata$data
...
RegDataLand <- RyggUtvalg$RegData
utvalgTxt <- RyggUtvalg$utvalgTxt


#Tidsrom	Det vil bare være hensiktsmessig å velge hele år.
#Kanskje vi bør ha mulighet for å velge flere år?
aardata <- aarvalg(RegDataLand, OpAar)
RegDataLand <- aardata$data
aartxt <- aardata$txt	

#Unn: 101627, St Olav: 105783 og 105899 (Namsos), SKDE: 105678, Lillehammer: 111185, Gjøvik: 111150
#reshID <- 105783
#RegDataLand$Antibiotika <- factor(RegDataLand$Antibiotika, levels=0:1)

indAvd <- which(as.numeric(RegDataLand$AvdReshID)==reshID)
RegDataAvd <- RegDataLand[indAvd, ]
Avdeling <- switch(enhetsUtvalg,
		'0' = 'NKR',
		'1' = as.character(RegDataAvd$ShNavn)[1])
indResten <- which(as.numeric(RegDataLand$AvdReshID)!=reshID)
RegDataRL <- RegDataLand[indResten,]

Ntot <- dim(RegData)[1]
NLand <- dim(RegDataLand)[1]
NResten <- length(indResten)
NAvd <- dim(RegDataAvd)[1]
#register <- 'NKR'	ikke i bruk?
dato <- strptime(RegDataLand$OpDato,format="%d.%m.%Y")
datomin <- min(dato)
datomax <- max(dato)
...
\Sexpr{datomax}. \Sexpr{if (enhetsUtvalg==1) {paste0('Siste registrerte inngrep ved ',Avdeling,
'ble utført ', max(strptime(RegDataAvd$OpDato, '%d.%m.%Y')))}.
\par 
\clearpage
...
\Sexpr{if (enhetsUtvalg ==1)  {paste0('pasienter operert ved Avdeling 
...
AlderFord <- RyggFigAndeler(RegData=RegDataLand, valgtVar='Alder', datoFra=datoFra, datoTil=datoTil, 
			reshID=reshID, enhetsUtvalg=enhetsUtvalg, outfile='FigAlderFord.pdf', tittel=0)

#	AldMSDAvd <- c(round(mean(RegDataAvd$Alder, na.rm=T),1), round(sd(RegDataAvd$Alder, na.rm=T)/sqrt(NAvd),1))
#	AldMSDRL <- c(round(mean(RegDataRL$Alder, na.rm=T),1), round(sd(RegDataRL$Alder, na.rm=T)/sqrt(NResten),1))
	if (enhetsUtvalg == 0) {#Alder <- rbind(AlderFord$Andeler[1,], AlderFord$Antall[1,])
	                        Alder <- rbind(paste(round(AlderFord$Andeler[1,], 1), '%', sep=''), AlderFord$Antall[1,])
	                        rownames(Alder) <- c('Andeler','Antall')}
	if (enhetsUtvalg == 1) {Alder <- AlderFord$Andeler}
      des <- 
	colnames(Alder) <- AlderFord$GruppeTekst
	xtable(Alder, caption=AlderFord$Tittel[1], label="tab:Alder", digits=c(1))

#round(length(which(RegDataAvd$Alder>65))/NAvd*100, 1)
#AndelOver65 <- sprintf('%.1f', 100*c(sum(RegDataAvd$Alder>65, na.rm=T)/NAvd, 
#									sum(RegDataRL$Alder>65, na.rm=T)/NResten))
@ 
...
%Andel pasienter over 65 år for \Sexpr{Avdeling} var \Sexpr{AndelOver65[1]}  mot 
...
SivStat <- cbind('Egen avdeling' = 
		c(paste(sprintf('%.1f', table(RegDataAvd$SivilStatus, useNA='always')/NAvd*100),'%', sep=''), NAvd ),
			'Landet for øvrig' = 
		c(paste(sprintf('%.1f', table(RegDataRL$SivilStatus, useNA='always')/NResten*100),'%', sep=''), NResten))
...
MorsmEgen <- table(RegDataAvd$Morsmal, useNA='a')/NAvd*100
MorsmResten <- table(RegDataRL$Morsmal, useNA='a')/NResten*100
...
<<'FigUtd', echo=FALSE>>=
Utd <- RyggFigAndeler(RegData=RegDataLand, valgtVar='Utd', datoFra=datoFra, datoTil=datoTil, 
...
RoykNum <- cbind('EgenAvd' = table(RegDataAvd$Roker), 'Resten'= table(RegDataRL$Roker) )
...
Royk <- cbind('Egen avdeling' = 
		paste(round(prop.table(ftable(RegDataAvd[,c('Kjonn','Roker')]),1)[,2]*100, 0), '%', sep=''),
			'Landet for øvrig' = 
		paste(round(prop.table(ftable(RegDataRL[,c('Kjonn','Roker')]),1)[,2]*100, 0), '%', sep=''))
...
%\Sexpr{round(prop.table(table(RegDataAvd$Roker))[2]*100, 1)}\% mot 
%\Sexpr{round(prop.table(table(RegDataRL$Roker))[2]*100, 1)}\% 
...
ArbAvd <- table(RegDataAvd$ArbstatusPre, useNA='a')*100/NAvd
ArbRL <- table(RegDataRL$ArbstatusPre, useNA='a')*100/NResten
...
indSykAvd <- which(RegDataAvd$ArbstatusPre %in% 6:10)
indSykRL <- which(RegDataRL$ArbstatusPre %in% 6:10)
GjsnSykAvd <- mean(RegDataAvd$SykemeldVarighPre[indSykAvd], na.rm=T)
GjsnSykRL <- mean(RegDataRL$SykemeldVarighPre[indSykRL], na.rm=T)
SdSykAvd <- sd(RegDataAvd$SykemeldVarighPre[indSykAvd], na.rm=T)
SdSykRL <- sd(RegDataRL$SykemeldVarighPre[indSykRL], na.rm=T)
MedSykAvd <- median(RegDataAvd$SykemeldVarighPre[indSykAvd], na.rm=T)
MedSykRL <- median(RegDataRL$SykemeldVarighPre[indSykRL], na.rm=T)
@ 
...
UforAvd <- round(table(RegDataAvd$UforetrygdPre, useNA='a')*100/NAvd, 1)
UforRL <- round(table(RegDataRL$UforetrygdPre, useNA='a')*100/NResten, 1)
...
ErstAvd <- round(table(RegDataAvd$ErstatningPre, useNA='a')*100/NAvd, 1)
ErstRL <- round(table(RegDataRL$ErstatningPre, useNA='a')*100/NResten, 1)
...
AndelTidlOp <- AndelStabelTid(RegData=RegDataLand, outfile='', variabel='TidlOp', 
...
<<'FigBMI', echo=FALSE>>=
BMIFord <- RyggFigAndeler(RegData=RegDataLand, valgtVar='BMI', datoFra=datoFra, datoTil=datoTil, 
...
SmRHAvd <- round(table(RegDataAvd$SymptVarighRyggHof, useNA='a')*100/NAvd, 1)
SmRHRL <- round(table(RegDataRL$SymptVarighRyggHof, useNA='a')*100/NResten, 1)
...
UtsRHAvd <- round(table(RegDataAvd$SympVarighUtstr, useNA='a')*100/NAvd, 1)
UtsRHRL <- round(table(RegDataRL$SympVarighUtstr, useNA='a')*100/NResten, 1)
...
\scalebox{\Sexpr{s1}}{
<<'FigASA', echo=FALSE>>=
ASA <- AndelStabelEgetLand(RegData=RegDataLand, variabel='ASA', outfile='', 
		reshID=reshID, OpAar=OpAar, kjonn=kjonn, hovedkat=hovedkat, tittel=0)
@
}
  \end{center}
...
RVAvd <- round(colSums(RegDataAvd[, RVvar], na.rm=T)*100/NAvd, 1)
RVRL <- round(colSums(RegDataRL[, RVvar], na.rm=T)*100/NResten, 1)
...
RFAvd <- round(colSums(RegDataAvd[, RFvar], na.rm=T)*100/NAvd, 1)
RFRL <- round(colSums(RegDataRL[, RFvar], na.rm=T)*100/NResten, 1)
AnnetAvd <- round(length(which(RegDataAvd$RfAnnetspes != 'NA'))*100/NAvd, 1)
AnnetRL <- round(length(which(RegDataRL$RfAnnetspes != 'NA'))*100/NResten, 1)
...
#RFNormAvd <- length(which(rowSums(RegDataAvd[, RFvar], na.rm=T) == 0))	#Dvs ingen funn!
RFNormAvd <- intersect(which(rowSums(RegDataAvd[, RFvar[2:11]], na.rm=T) == 0), 
		which(RegDataAvd$RfNormal == 1))
RFNormRL <- intersect(which(rowSums(RegDataRL[, RFvar[2:11]], na.rm=T) == 0), 
		which(RegDataRL$RfNormal == 1))
@ 
...
ind3mndAvd <- which(as.Date(strptime(RegDataAvd$OpDato, '%d.%m.%Y')) < Sys.Date() - 120)
ind12mndAvd <- which(as.Date(strptime(RegDataAvd$OpDato, '%d.%m.%Y')) < Sys.Date() - 400)
ind3mndRL <- which(as.Date(strptime(RegDataRL$OpDato, '%d.%m.%Y')) < Sys.Date() - 120)
ind12mndRL <- which(as.Date(strptime(RegDataRL$OpDato, '%d.%m.%Y')) < Sys.Date() - 400)

Svar3mndAvd <- round(sum(RegDataAvd$Utfylt3Mnd[ind3mndAvd])/length(ind3mndAvd)*100)
Svar12mndAvd <- round(sum(RegDataAvd$Utfylt12Mnd[ind12mndAvd])/length(ind12mndAvd)*100)
Svar3mndRL <- round(sum(RegDataRL$Utfylt3Mnd[ind3mndRL])/length(ind3mndRL)*100)
Svar12mndRL <- round(sum(RegDataRL$Utfylt12Mnd[ind12mndRL])/length(ind12mndRL)*100)
...
AntOpMndAvd <- AntSoyler(RegData=RegDataAvd, outfile=paste(figKat,'AntOpAvd.pdf', sep=''), egenavd=1, 
	variabel='Mnd', hovedkat=hovedkat, OpAar=OpAar, kjonn=kjonn, tittel=0, reshID=reshID)
AntOpMndLand <- AntSoyler(RegData=RegDataLand, outfile=paste(figKat,'AntOpLand.pdf', sep=''), egenavd=0, 
	hovedkat=hovedkat, OpAar=OpAar, reshID=reshID, tittel=0, variabel='Mnd')
@
...
Ved \Sexpr{Avdeling} er \Sexpr{round(length(which(RegDataAvd$Inngrep==0))/NAvd*100,1)} \% 
av inngrepene udefinerbare, mot \Sexpr{round(length(which(RegDataLand$Inngrep==0))/NLand*100,1)} \% 
for hele landet. 
...
HovedInngrep <- RyggFigAndeler(RegData=RegDataLand, valgtVar='HovedInngrep', datoFra=datoFra, datoTil=datoTil, 
			reshID=reshID, outfile='FigHovedInngrep.pdf', tittel=0)

#AntHovedInngrepAvd <- AntSoyler(RegData=RegDataAvd, outfile=paste(figKat,'HinngrAvd.pdf', sep=''), 
#	egenavd=1, OpAar=OpAar, reshID=reshID, variabel='HovedInngrep', tittel=0)
#AntHovedInngrepLand <- AntSoyler(RegData=RegDataLand, outfile=paste(figKat,'HinngrLand.pdf', sep=''), 
...
AntInngrAvd <- table(RegDataAvd$Inngrep)
AntHinngrAvd <- table(RegDataAvd$HovedInngrep)[HKatdata$inngrHinn$Hnr+1]
AntInngrRL <- table(RegDataRL$Inngrep)
AntHinngrRL <- table(RegDataRL$HovedInngrep)[HKatdata$inngrHinn$Hnr+1]
...
\scalebox{\Sexpr{s1}}{
<<'FigOpKat', echo=FALSE>>=
AndelOpKat <- AndelStabelEgetLand(RegData=RegDataLand, outfile='', variabel='OpKat',
		kjonn=kjonn, hovedkat=hovedkat, OpAar=OpAar, reshID=reshID, tittel=0)
		
#HastTest	<- prop.test(c(table(RegDataAvd$OpKat)[3],table(RegDataRL$OpKat)[3]),
#		c(sum(table(RegDataAvd$OpKat)), sum(table(RegDataRL$OpKat))))
...
LiggetidFord <- RyggFigAndeler(RegData=RegDataLand, valgtVar='Liggedogn', datoFra=datoFra, datoTil=datoTil, 
			reshID=reshID, outfile='FigLiggetidFord.pdf', tittel=0)

#LiggetidFord <- AndelSoyler(RegData=RegDataLand, variabel='Liggedogn', 
#outfile=paste(figKat,'LiggetidFord.pdf', sep=''), 
#		reshID=reshID, OpAar=OpAar, kjonn=kjonn, hovedkat=hovedkat, tittel=0)


#LiggetidBox <- BoxTid(RegData=RegDataLand, outfile=paste(figKat,'LiggetidBox.pdf', sep=''), 
#reshID=reshID, variabel='Liggedogn', kjonn=kjonn, hovedkat=hovedkat, ktr=ktr, tittel=0)
LiggetidBox <- GjsnTid(RegData=RegDataLand,  outfile=paste(figKat,'LiggetidBox.pdf', sep=''), 
...
UtData <- AndelTidLinjer(RegData=RegDataLand, outfile='', reshID=reshID, 
...
#Hele landet:
NMangel <- NLand- sum(table(RegDataLand$Antibiotika))

#Egen:
NtotEgen<- round(sum(table(RegDataAvd$OpAar)), 1)	
Antibiotika <- tapply(RegDataAvd$Antibiotika, RegDataAvd$OpAar, FUN=sum, na.rm=T)	#Antibiotika <-ftable(list(RegDataAvd$Antibiotika, RegDataAvd$OpAar))
...
#dum <- BoxTid(RegData=RegDataLand, outfile=paste(figKat,'EQ5DPre.pdf', sep=''), reshID=reshID, 
#		variabel='EQ5DPre', kjonn=kjonn, hovedkat=hovedkat, ktr=ktr, tittel=0)
#dum <- BoxTid(RegData=RegDataLand, outfile=paste(figKat,'OswPre.pdf', sep=''), reshID=reshID, 
#		variabel='OswTotPre', kjonn=kjonn, hovedkat=hovedkat, ktr=ktr, tittel=0)
#dum <- BoxTid(RegData=RegDataLand, outfile=paste(figKat,'SmBePre.pdf', sep=''), reshID=reshID, 
#		variabel='SmBePre', kjonn=kjonn, hovedkat=hovedkat, ktr=ktr, tittel=0)
#dum <- BoxTid(RegData=RegDataLand, outfile=paste(figKat,'SmRyPre.pdf', sep=''), reshID=reshID, 
#		variabel='SmRyPre', kjonn=kjonn, hovedkat=hovedkat, ktr=ktr, tittel=0)
		
		
dum <- GjsnTid(RegData=RegDataLand,  outfile=paste(figKat,'EQ5DPre.pdf', sep=''), 
		valgtVar='EQ5DPre', ktr=ktr, tittel=0, valgtMaal='', enhetsUtvalg=1, reshID=reshID)
dum <- GjsnTid(RegData=RegDataLand,  outfile=paste(figKat,'OswPre.pdf', sep=''), 
		valgtVar='OswTotPre', ktr=ktr, tittel=0, valgtMaal='', enhetsUtvalg=1, reshID=reshID)
dum <- GjsnTid(RegData=RegDataLand,  outfile=paste(figKat,'SmBePre.pdf', sep=''), 
		valgtVar='SmBePre', ktr=ktr, tittel=0, valgtMaal='', enhetsUtvalg=1, reshID=reshID)
dum <- GjsnTid(RegData=RegDataLand,  outfile=paste(figKat,'SmRyPre.pdf', sep=''), 
...
dum <- EndrPre(RegData=RegDataLand, outfile=paste(figKat,'OswEndrPre.pdf', sep=''), 
	reshID=reshID, OpAar=OpAar, variabel='Oswestry', kjonn=kjonn, hovedkat=hovedkat, 
	ktr=ktr, tittel=0)
dum <- EndrPre(RegData=RegDataLand, outfile=paste(figKat,'EQ5DEndrPre.pdf', sep=''), 
	reshID=reshID, OpAar=OpAar, variabel='EQ5D', kjonn=kjonn, hovedkat=hovedkat, 
	ktr=ktr, tittel=0)
dum <- EndrPre(RegData=RegDataLand, outfile=paste(figKat,'SmBeinEndrPre.pdf', sep=''), 
	reshID=reshID, OpAar=OpAar, variabel='SmBein', kjonn=kjonn, hovedkat=hovedkat, 
	ktr=ktr, tittel=0)
dum <- EndrPre(RegData=RegDataLand, outfile=paste(figKat,'SmRyggEndrPre.pdf', sep=''), 
	reshID=reshID, OpAar=OpAar, variabel='SmRygg', kjonn=kjonn, hovedkat=hovedkat, 
	ktr=ktr, tittel=0)
@
...
dum <- FordPrePost(RegData=RegDataLand, outfile=paste(figKat,'OswestryPP.pdf', sep=''), 
	reshID=reshID, egenavd=egenavd, variabel='Oswestry', plotType='S', OpAar=OpAar, kjonn=kjonn, hovedkat=hovedkat, ktr=ktr, tittel=0)
#dum <- BoxTid(RegData=RegDataLand, outfile=paste(figKat,'OswEndr.pdf', sep=''), reshID=reshID, 
#	variabel='OswEndr', kjonn=kjonn, hovedkat=hovedkat, ktr=ktr, tittel=0)
dum <- GjsnTid(RegData=RegDataLand,  outfile=paste(figKat,'OswEndr.pdf', sep=''), 
		valgtVar='OswEndr', ktr=ktr, tittel=0, valgtMaal='', enhetsUtvalg=1, reshID=reshID)
dum <- Osw_over30pst(RegData=RegDataLand, outfile=paste(figKat,'Osw_over30pst.pdf', sep=''),
		hovedkat=hovedkat, ktr=ktr, tidlOp=tidlOp, shgr=0, OpAar=OpAar, kjonn=kjonn, reshID=reshID, tittel=0)
dum <- SoyleSml(RegData=RegDataLand, outfile=paste(figKat,'OswEndrSml.pdf', sep=''), 
	reshID=reshID, variabel='OswEndr', tidlOp=tidlOp, OpAar=OpAar, kjonn=kjonn, shgr=0, 
...
#FordPrePost(RegData=RegData, outfile='figurer/EQ5DPP.pdf', reshID=reshID,
#	egenavd=egenavd, variabel='Oswestry', plotType='', OpAar=OpAar, kjonn=kjonn, hovedkat=hovedkat, ktr=ktr, tittel=0)
#dum <- BoxTid(RegData=RegDataLand, outfile=paste(figKat,'EQ5DEndr.pdf', sep=''), reshID=reshID, 
#	variabel='EQ5DEndr', kjonn=kjonn, hovedkat=hovedkat, ktr=ktr, tittel=0)
dum <- GjsnTid(RegData=RegDataLand,  outfile=paste(figKat,'EQ5DEndr.pdf', sep=''), 
		valgtVar='EQ5DEndr', ktr=ktr, tittel=0, valgtMaal='', enhetsUtvalg=1, reshID=reshID)
dum <- SoyleSml(RegData=RegDataLand, outfile=paste(figKat,'EQ5DEndrSml.pdf', sep=''), 
	reshID=reshID, variabel='EQ5DEndr', tidlOp=tidlOp, OpAar=OpAar, kjonn=kjonn, shgr=0, hovedkat=hovedkat, ktr=ktr, just=0, tittel=0)
@
...
dum <- FordPrePost(RegData=RegDataLand, outfile=paste(figKat,'SmRyggPP.pdf', sep=''), 
	reshID=reshID, egenavd=egenavd, variabel='SmRygg', plotType=plotType, OpAar=OpAar, 
	kjonn=kjonn, hovedkat=hovedkat, ktr=ktr, tittel=0)
#dum <- BoxTid(RegData=RegDataLand, outfile=paste(figKat,'SmRyggSml.pdf', sep=''), reshID=reshID, 
#	variabel='SmRyggEndr', kjonn=kjonn, hovedkat=hovedkat, ktr=ktr, tittel=0)
dum <- GjsnTid(RegData=RegDataLand,  outfile=paste(figKat,'SmRyggSml.pdf', sep=''), 
...
dum <- FordPrePost(RegData=RegDataLand, outfile=paste(figKat,'SmBeinPP.pdf', sep=''), reshID=reshID, egenavd=egenavd, variabel='SmBein', plotType=plotType, OpAar=OpAar, kjonn=kjonn, hovedkat=hovedkat, ktr=ktr, tittel=0)
#dum <- BoxTid(RegData=RegDataLand, outfile=paste(figKat,'SmBeinSml.pdf', sep=''), reshID=reshID, 
#	variabel='SmBeinEndr', kjonn=kjonn, hovedkat=hovedkat, ktr=ktr, tittel=0)
dum <- GjsnTid(RegData=RegDataLand,  outfile=paste(figKat,'SmBeinSml.pdf', sep=''), 
...
\scalebox{\Sexpr{s1}}{
<<'FigNytte', echo=FALSE>>=
	Nytte <- AndelStabelTid(RegData=RegDataLand, outfile='', variabel='Nytte', 
...
<<'FigFornoyd', echo=FALSE>>=
	Fornoyd <- AndelStabelTid(RegData=RegDataLand, outfile='', variabel='Fornoyd', kjonn=kjonn, 
...
<<'FigFrisk', echo=FALSE>>=	 #fig=TRUE, 
	Frisk3mnd <- AndelTidLinjer(RegData=RegDataLand, variabel='Friskmeldt', 
		outfile=paste(figKat,'Frisk1.pdf', sep=''), reshID=reshID, tidlOp=tidlOp, kjonn=kjonn, hovedkat=hovedkat, ktr=1, medTall=mT, tittel=0)
	Frisk12mnd <- AndelTidLinjer(RegData=RegDataLand, variabel='Friskmeldt', 
