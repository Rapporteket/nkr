...
\Sexpr{datomax}. \Sexpr{if (enhetsUtvalg==1) {paste0('Siste registrerte inngrep ved ', shtxt,
'ble utført ', max(strptime(RegDataHoved$OpDato, '%d.%m.%Y')))}}.
\par 
\clearpage
...
\Sexpr{if (enhetsUtvalg ==1)  {paste0('pasienter operert ved ', shtxt, 
...
if (medSml ==0) {
SivStat <- cbind('Antall'= c(table(opdataLand$SivilStatus, useNA='always'), Ntot),
		'Andeler'= 
		c(paste(round(table(opdataLand$SivilStatus, useNA='always')/Ntot*100,1),'%', sep=''), ''))
...
\Sexpr{if (medSml=0) {paste0('Det er ', Royk[1], '\% av mennene og ', Royk[2], '\% av kvinnene som røyker. 
...
if (enhetsUtvalg == 0) {
Ufor <- paste(round(table(opdataLand$UforetrygdPre, useNA='a')*100/Ntot, 1), '%', sep='')
...
if (enhetsUtvalg == 0) {
Erst<- paste(round(table(opdataLand$ErstatningPre, useNA='a')*100/Ntot, 1), '%', sep='')	#, Ntot)
...
det aktuelle året\Sexpr{if (medSml==1) {', mens det hvite merket angir andel pasienter som har første 
gangs operasjon i resten av landet'}}.

<<'FigTidlOp', echo=FALSE>>=
AndelTidlOp <- RyggAndelStabelTid(RegData=RegData, outfile='TidlOp.pdf', valgtVar='TidlOp', hovedkat=hovedkat,
...
if (medSml == 0) {
SmRHnum <- round(table(opdataLand$SymptVarighRyggHof, useNA='a')*100/Ntot, 1)
...
UtsRHnum <- round(table(opdataLand$SympVarighUtstr, useNA='a')*100/Ntot, 1)
...
#dum <- LinjerTidRHF(RegData=opdataLand, variabel='VarighUtstr', outfile='VarighUtstrRHF.pdf', hovedkat=99, 
#		kjonn=0, tidlOp=0, tittel=0) 
#dum <- LinjerTidRHF(RegData=opdataLand, variabel='VarighRyggHof', outfile='VarighRyggHofRHF.pdf', hovedkat=99, 
#		kjonn=0, tidlOp=0, tittel=0) 
...
var \Sexpr{round(sum(table(opdataLand$ASA)[1:2])/Ntot*100, 1)}\%. 
...
Totalt er \Sexpr{round(length(which(opdataLand$Inngrep==0))/Ntot*100,1)} \% 
...
#AntHovedInngrepLand <- AntSoyler(RegData=opdataLand, libkat=libkat, outfile='figurer/HinngrLand.pdf', 
...
HovedInngrep <- RyggFigAndeler(RegData=RegData, valgtVar='HovedInngrep', datoFra=datoFra, datoTil=datoTil, 
			reshID=reshID, outfile='HovedInngrep.pdf', tittel=0)
...
AntHovedInngrep <- cbind('Antall' = AntHovedInngrepLand$Antall, 
					'Andeler' = paste(round(AntHovedInngrepLand$Antall/Ntot*100,1),'%', sep='')
					)
...
<<'Antibiotika', results='asis', echo=FALSE>>=
Antibiotika <- AndelTidLinjer(RegData=RegData, outfile='Antibiotika.pdf', reshID=reshID, 
	erMann=erMann, hovedkat=hovedkat, variabel='Antibiotika', tittel=0)	
...
#Hele landet:
NMangel <- Ntot - sum(table(opdataLand$Antibiotika))
...
dum <- RyggFigGjsnBox(RegData=RegData, outfile='OswEndrPre.pdf', 
	reshID=reshID, OpAar=OpAar, variabel='Oswestry', erMann=erMann, hovedkat=hovedkat, 
	ktr=ktr, tittel=0)
dum <- RyggFigGjsnBox(RegData=RegData, outfile='EQ5DEndrPre.pdf', 
	reshID=reshID, OpAar=OpAar, variabel='EQ5D', erMann=erMann, hovedkat=hovedkat, 
	ktr=ktr, tittel=0)
dum <- RyggFigGjsnBox(RegData=RegData, outfile='SmBeinEndrPre.pdf', 
	reshID=reshID, OpAar=OpAar, variabel='SmBein', erMann=erMann, hovedkat=hovedkat, 
	ktr=ktr, tittel=0)
dum <- RyggFigGjsnBox(RegData=RegData, outfile='SmRyggEndrPre.pdf', 
	reshID=reshID, OpAar=OpAar, variabel='SmRygg', erMann=erMann, hovedkat=hovedkat, 
	ktr=ktr, tittel=0)
@
...
dum <- FordPrePost(RegData=RegData, outfile='OswestryPP.pdf', 
	reshID=reshID, egenavd=egenavd, variabel='Oswestry', plotType='S', OpAar=OpAar, erMann=erMann, hovedkat=hovedkat, ktr=ktr, tittel=0)
#dum <- BoxTid(RegData=RegData, outfile='OswEndr.pdf', reshID=reshID, 
#	variabel='OswEndr', erMann=erMann, hovedkat=hovedkat, ktr=ktr, tittel=0)
dum <- RyggFigGjsnTid(RegData=RegData,  outfile='OswEndr.pdf', 
		valgtVar='OswEndr', ktr=ktr, tittel=0, valgtMaal='', enhetsUtvalg=1, reshID=reshID)
dum <- Osw_over30pst(RegData=RegData, outfile='Osw_over30pst.pdf',
...
dum <- SoyleSml(RegData=RegData, outfile='OswEndrSml.pdf', 
	reshID=reshID, variabel='OswEndr', tidlOp=tidlOp, OpAar=OpAar, erMann=erMann, shgr=0, 
...
\begin{figure}[h] 
\centerline{
  \scalebox{\Sexpr{s2}}{\includegraphics{\Sexpr{ 'Osw_over30pst.pdf'}}}
...
dum <- RyggFigGjsnTid(RegData=RegData,  outfile='EQ5DEndr.pdf', 
...
dum <- SoyleSml(RegData=RegData, outfile='EQ5DEndrSml.pdf', 
	reshID=reshID, variabel='EQ5DEndr', tidlOp=tidlOp, OpAar=OpAar, erMann=erMann, shgr=0, hovedkat=hovedkat, ktr=ktr, just=0, tittel=0)
@

\begin{figure}[h] 
\centerline{
  \scalebox{\Sexpr{s2}}{\includegraphics{\Sexpr{ 'EQ5DEndr.pdf'}}}
  \scalebox{\Sexpr{s2}}{\includegraphics{\Sexpr{ 'EQ5DEndrSml.pdf'}}}
  }
  \caption{\Sexpr{paste('Endring i EQ-5D, ', ktrtxt, 'etter operasjon'}}
  \label{fig:EQ5DEndr}
\end{figure}

Figur \ref{fig:EQ5DEndr} viser endring i EQ-5D score 3 og 12 mnd. etter operasjon hos pasienter operert ved \Sexpr{shtxt} og i landet for øvrig for hvert år.


\subsection{Ryggsmerter}
Skala for ryggsmerter går fra 0 til 10, hvor 0 betegner ingen smerte.
<<'FigSmRy', echo=FALSE>>=	 #fig=TRUE, 
dum <- FordPrePost(RegData=RegData, outfile='SmRyggPP.pdf', 
	reshID=reshID, egenavd=egenavd, variabel='SmRygg', plotType=plotType, OpAar=OpAar, 
	erMann=erMann, hovedkat=hovedkat, ktr=ktr, tittel=0)
#dum <- BoxTid(RegData=RegData, outfile='SmRyggSml.pdf', reshID=reshID, 
#	variabel='SmRyggEndr', erMann=erMann, hovedkat=hovedkat, ktr=ktr, tittel=0)
dum <- RyggFigGjsnTid(RegData=RegData,  outfile='SmRyggSml.pdf', 
		valgtVar='SmRyggEndr', ktr=ktr, tittel=0, valgtMaal='', enhetsUtvalg=1, reshID=reshID)
@

\begin{figure}[h] 
\centerline{
  \scalebox{\Sexpr{s2}}{\includegraphics{\Sexpr{ 'SmRyggPP.pdf'}}}
  \scalebox{\Sexpr{s2}}{\includegraphics{\Sexpr{ 'SmRyggSml.pdf'}}}
  }
  \caption{\Sexpr{paste('Endring i ryggsmerter, ', ktrtxt, 'etter operasjon', sep='')}}
  \label{fig:SmRygg}
\end{figure}

Figur \ref{fig:SmRygg} viser hhv fordeling av ryggsmerter før og etter operasjon, samt endring i ryggsmerter \Sexpr{ktrtxt} etter operasjon hos pasienter operert ved \Sexpr{shtxt} og i landet for øvrig for hvert år.
...
dum <- RyggFigGjsnTid(RegData=RegData,  outfile='SmBeinSml.pdf', 
		valgtVar='SmBeinEndr', ktr=ktr, tittel=0, valgtMaal='', enhetsUtvalg=1, reshID=reshID)
@

\begin{figure}[h] 
\centerline{
  \scalebox{\Sexpr{s2}}{\includegraphics{\Sexpr{ 'SmBeinPP.pdf'}}}
  \scalebox{\Sexpr{s2}}{\includegraphics{\Sexpr{ 'SmBeinSml.pdf'}}}
  }
  \caption{\Sexpr{paste('Endring i beinsmerter, ', ktrtxt, 'etter operasjon', sep='')}}
  \label{fig:SmBein}
\end{figure}

Figur \ref{fig:SmBein} viser hhv fordeling av ryggsmerter før og etter operasjon, samt endring i ryggsmerter \Sexpr{ktrtxt} etter operasjon hos pasienter operert ved \Sexpr{shtxt} og i landet for øvrig for hvert år.
...
\scalebox{\Sexpr{s1}}{
<<'FigNytte', echo=FALSE>>=
	Nytte <- AndelStabelTid(RegData=RegData, outfile='', variabel='Nytte', 
	erMann=erMann, hovedkat=hovedkat, tidlOp=tidlOp, egenavd=egenavd, ktr=ktr, 
...
<<'FigFornoyd', echo=FALSE>>=
	Fornoyd <- AndelStabelTid(RegData=RegData, outfile='', variabel='Fornoyd', erMann=erMann, 
