...
dum <- RyggFigAndelTid(valgtVar='SympVarighUtstr', RegData=RegData, 
                       hovedkat=1,outfile='VarighUtstrTidPro.pdf')
dum <- RyggFigAndelTid(valgtVar='SympVarighUtstr', RegData=RegData, 
                       hovedkat=8,outfile='VarighUtstrTidSS.pdf')
...
Figur \ref{fig: 
\begin{figure}[h] 
      \scalebox{\Sexpr{s1}}{\includegraphics{VarighUtstrTidPro.pdf}}
      \caption{Prolapspasienter som har utstrålende smerter i mer enn ett år før operasjonen, utvikling over tid.}
      \label{fig:VarighSmerteUtstrAvdPro}
\end{figure}

\begin{figure}[h] 
      \scalebox{\Sexpr{s1}}{\includegraphics{VarighUtstrTidSS.pdf}}
...
De hyppigste tilstandene pasienter opereres for er prolapskirurgi (\Sexpr{round(HovedInngrep$Andeler[1,2])} \%) og spinal stenose (\Sexpr{AndelSSrappaar} \%). En andel av de som har spinal stenose har også en forskyvning mellom ryggvirvlene (Degenerativ spondylolistese). I faglitteraturen er det sprikende anbefalinger for om denne undergruppen skal ha tilleggsbehandling med avstivningsoperasjon (fusjonskirurgi).
...
\begin{figure}[h] 
\centering \includegraphics[width=\Sexpr{figKrymp1}\textwidth]{FigLiggetidFord.pdf}
...
dum <- RyggFigGjsnGrVar(RegData=RegData, outfile='OswEndrAvdPro.pdf', Ngrense = 10,
                      aar=2014:2015, valgtVar='OswEndr', hovedkat=1, ktr=ktr, tittel=0)
dum <- RyggFigGjsnGrVar(RegData=RegData, outfile='OswEndrAvdSS.pdf', Ngrense = 10,
                      aar=2014:2015, valgtVar='OswEndr', hovedkat=8, ktr=ktr, tittel=0)


dum <- RyggFigAndelTid(RegData=RegData, outfile='Osw20ProTid.pdf', 
                      aar=2010:2015, valgtVar='OswEndr20', hovedkat=1, ktr=ktr, tittel=0)
dum <- RyggFigAndelTid(RegData=RegData, outfile='Osw30SSTid.pdf', 
...
\begin{figure}[h] 
      \centerline{\scalebox{\Sexpr{s1}}{\includegraphics{OswEndrAvdPro.pdf}}}
      \caption{Gjennomsnittlig endring av ODI per avdeling for prolaps}}
      \label{fig:OswEndrAvdPro}
\end{figure}

\begin{figure}[h] 
      \centerline{\scalebox{\Sexpr{s1}}{\includegraphics{OswEndrAvdSS.pdf}}}
...
\begin{figure}[h] 
      \centerline{\scalebox{\Sexpr{s1}}{\includegraphics{Osw20ProTid.pdf}}}
...
\begin{figure}[h] 
      \centerline{\scalebox{\Sexpr{s1}}{\includegraphics{Osw30SS.pdf}}}
...
<<FigNytte, echo=FALSE>>=
NyttePro <- RyggFigAndelStabelTid(RegData=RegData, aar=2010:2015, outfile='FigNyttePro.pdf', valgtVar='Nytte', 
                                  hovedkat=1, ktr=ktr, tittel=0)

NytteSS <- RyggFigAndelStabelTid(RegData=RegData, aar=2010:2015, outfile='FigNytteSS.pdf', valgtVar='Nytte',
...
ODIProOpKat <- round(prop.table(table(RegData1aar$ODIendr[indPro1aar]>20, RegData1aar$OpKat[indPro1aar]),2)*100,1)
ODIProTidlOp <- round(prop.table(table(RegData1aar$ODIendr[indPro1aar]>20, RegData1aar$TidlOpr[indPro1aar]==4),2)*100,1)
ODIProTidlOpAnt3 <- round(prop.table(table(RegData1aar$ODIendr[indPro1aar]>20, RegData1aar$TidlOprAntall[indPro1aar]>2),2)*100,1)

indSS1aar <- intersect(indSS, which(RegData$OpAar==rappAar))
AndelOhjSS <- round(prop.table(table(RegData$OpKat[indSS1aar]))*100,1)
ODISSTidlOpAnt3 <- round(prop.table(table((RegData$ODIpst)[indSS1aar]>=30, 
                                          RegData$TidlOprAntall[indSS1aar]>2),2)*100,1)
