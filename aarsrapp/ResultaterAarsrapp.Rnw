...
\begin{figure}[h] 
\centerline{
  \scalebox{\Sexpr{figKrymp1}}{\includegraphics{FigLiggetidFord.pdf}}
  }
...
dum <- RyggFigGjsnBox(RegData=RegData, outfile='OswEndrPro.pdf', 
	aar=2010:2015, valgtVar='OswEndrPre', hovedkat=1, ktr=ktr, tittel=0)
dum <- RyggFigGjsnBox(RegData=RegData, outfile='OswEndrSS.pdf', 
	aar=2010:2015, valgtVar='OswEndrPre', hovedkat=8, ktr=ktr, tittel=0)
	
dum <- RyggFigGjsnBox(RegData=RegData, outfile='EQ5DEndrPre.pdf', 
	aar=aar, valgtVar='EQ5DEndrPre', hovedkat=hovedkat, ktr=ktr, tittel=0)

dum <- RyggFigGjsnBox(RegData=RegData, outfile='SmBeinEndrPre.pdf', 
	aar=aar, valgtVar='SmBeinEndrPre', hovedkat=hovedkat, ktr=ktr, tittel=0)
dum <- RyggFigGjsnBox(RegData=RegData, outfile='SmRyggEndrPre.pdf', 
	aar=aar, valgtVar='SmRyggEndrPre', hovedkat=hovedkat, ktr=ktr, tittel=0)
@
...
\textit{Disse har vi ikke: Figur \ref{fig:OswEndrSml} viser andel pasienter med klinisk signifikant forbedring av ODI (dvs. minst 30$\%$) 12 måneder etter operasjon. Høyre del av figuren viser gjennomsnittlig endring 12 måneder for hver avdeling. }
Merk at resultatene \textit{ikke} er justert for forskjeller i pasientpopulasjonene.


\begin{figure}[h] 
\centerline{
  \scalebox{\Sexpr{s2}}{\includegraphics{OswEndrProlaps.pdf}}
...
\subsection{ Resultater etter rygg og nakkekirurgi nasjonalt og per sykehus fra 2010 til \Sexp{rappAar}}
\texbf{Denne overskrifta virker litt malplassert. Og det gjør flere av de kommende overskrifter også. Det henger fortsatt igjen fra todelinga av resultatkapitlet at det er rotete. Håper det blir lettere å rydde nå når alt er oppdatert.}
...
\textif{Vurderes på nytt: alvorlig til minimal for gjennomsnittspasienten.} \\
...
Liggetiden på sykehus i forbindelse med prolapsoperasjon har gått ned med \Sexpr{NedgLiggetidPro} døgn fra 2010 til \Sexpr{rappAar}.
...
BeinsmLavPre <- RyggFigAndelerGrVar(RegData=RegData1aar, hovedkat=1,  opKat=1, tidlOp=4, valgtVar='BeinsmLavPre', 
                                 outfile='FigBeinsmLavPre.pdf')
@
...
\begin{figure}[ht]
	...
	\caption{\label{fig:BeinsmLavPre}  Andel pasienter med lite bensmerter (\leq 2) operert for prolaps.}
\end{figure}
...
Osw48 <- RyggFigAndelerGrVar(RegData=RegData, aar=2015, valgtVar='Osw48', 
			hovedkat = 1, opKat=1, tidlOp=4, outfile='FigOsw48.pdf', tittel=0)

OswEndrLav <- RyggFigAndelerGrVar(RegData=RegData, aar=2015, valgtVar='OswEndrLav', 
			hovedkat = 1, opKat=1, tidlOp=4, outfile='FigOswEndrLav.pdf', tittel=0)

OswEndr20 <- RyggFigAndelerGrVar(RegData=RegData, aar=2015, valgtVar='OswEndr20', 
			hovedkat = 1, opKat=1, tidlOp=4, outfile='FigOswEndr20.pdf', tittel=0)

OswEndr30pstSS <- RyggFigAndelerGrVar(RegData=RegData, aar=2015, valgtVar='OswEndr30pst', 
			hovedkat = 1, opKat=1, tidlOp=4, outfile='FigOswEndr30pstSS.pdf', tittel=0)
@
...
<<FigurInf, echo=FALSE>>=
NNNKpInf3MndPro <- RyggFigAndelerGrVar(RegData=RegData1aar, valgtVar='KpInf3Mnd', 
...
dum <- RyggFigAndelerGrVar(RegData = RegData1aar, valgtVar = 'degSponFusj', outfile = 'FigdegSponFusj.pdf')
@
