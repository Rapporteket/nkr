...
dum <- RyggFigAndelTid(RegData=RegData, outfile='Osw20Pro.pdf', 
                      aar=2010:2015, valgtVar='OswEndr20', hovedkat=1, ktr=ktr, tittel=0)
dum <- RyggFigGjsnBox(RegData=RegData, outfile='Osw30SS.pdf', 
...
Figur \ref{fig:Nytte} viser ''andel pasienter med klinisk signifikant forbedring av ODI 12 måneder etter operasjon. Høyre del av figuren viser gjennomsnittlig endring 12 måneder for hver avdeling.''

\tekstit{En figur for hver}
\begin{itemize}
\item gjennomsnittlig endring av ODI per avdeling for prolaps
\item gjennomsnittlig endring av ODI per avdeling for  SS
\end{itemize}


\begin{figure}[h] 
      \centerline{\scalebox{\Sexpr{s1}}{\includegraphics{Osw20Pro.pdf}}}
      \caption{Andel med klinisk signifikant endring i Oswestry (>20 poeng) for prolapspasienter, ett år etter operasjon.}}
      \label{fig:OswEndr20TidPro}
\end{figure}
