...
\Sexpr{dim(tabAvdN)[1]-1} avdelinger som registrerer og at det i perioden 2010 til \Sexpr{aar} totalt er registrert \Sexpr{Ntot} 
...
<<ResAlder, results='asis',eval=FALSE>>=
AlderFord <- RyggFigAndeler(RegData=RegData1aar, valgtVar='Alder', datoFra=datoFra, datoTil=datoTil, enhetsUtvalg=0, outfile='FigAlderFord.pdf', tittel=0)
	...
	Alder <- rbind(paste0(round(AlderFord$Andeler[1,], 1), '%'), AlderFord$Antall[1,])
...
Figur \ref{fig:Alder} viser aldersfordeling for alle pasienter i \Sexpr{aar}.


\begin{figure}[ht]
	\centering \includegraphics[width= \Sexpr{figKrymp1}\textwidth]{FigAlderFord.pdf}
	\caption{\label{fig:Alder} \Sexpr{paste0(AlderFord$Tittel,', ',aar)}}
\end{figure}
...
pasienter (inkl. samisk) var \Sexpr{AndelFremmedspraak[1]}\% . 
...
<<FigUtd, echo=FALSE>>=
Utd <- RyggFigAndeler(RegData=RegData1aar, valgtVar='Utd', datoFra=datoFra, datoTil=datoTil, 
			enhetsUtvalg=enhetsUtvalg, outfile='FigUtd.pdf', tittel=0)
...
Ufor <- paste(round(table(RegData1aar$UforetrygdPre, useNA='a')*100/Ntot1aar, 1), '%', sep='')
...
<<FigTidlOp, echo=FALSE>>=
	AndelTidlOp <- RyggFigAndelStabelTid(RegData=RegData, outfile='TidlOp.pdf', valgtVar='TidlOp', 
	                                     hovedkat=hovedkat,
							enhetsUtvalg=enhetsUtvalg, tittel=0)
	...
	AndelerTidlOp <- AndelerSisteAar[2:4]/sum(AndelerSisteAar[2:4])*100
...
Figur \ref{fig:BMI} viser fordeling av BMI for alle pasienter i \Sexpr{aar}. 
...
\begin{figure}[h] 
  \scalebox{\Sexpr{s1}}{\includegraphics{VarighUtstrAvd.pdf}}
...
\begin{figure}[h] 
  \scalebox{\Sexpr{s1}}{\includegraphics{VarighRyggHofAvd.pdf}}
...
var \Sexpr{round(sum(table(RegData1aar$ASA)[1:2])/Ntot1aar*100, 1)}\%. 
...
xtable(RV, caption=paste0('Radiologisk vurdering, ',aar), 
...
txtRFnorm <- ifelse (length(RFNorm)>0,
       paste0("``Normalt'' er registrert som eneste billedfunn hos ", length(RFNorm), " pasienter.
...
Totalt er \Sexpr{round(length(which(RegData1aar$Inngrep==0))/Ntot1aar*100,1)} \% 
...
xtable(HovedInngrepTab, align=c('l','r','r'), caption=paste0('Fordeling av hovedinngrep, ', aar), label="tab:AntHovedInngrep", digits=1)
...
LiggetidBox <- RyggFigGjsnBox(RegData=RegData,  reshID=0, outfile='LiggetidBox.pdf', 
		valgtVar='Liggedogn', ktr=ktr, tittel=0, valgtMaal='', enhetsUtvalg=enhetsUtvalg)
@
...
<<Antibiotika, results='asis', echo=FALSE, eval=FALSE>>=
	Antibiotika <- RyggFigAndelerTid(RegData=RegData, outfile='Antibiotika.pdf', enhetsUtvalg=enhetsUtvalg,
		hovedkat=hovedkat, variabel='Antibiotika', tittel=0)	#AndelTidLinjer
...
<<FigPre, echo=FALSE>>=	 #fig=TRUE, 
	...
	dum <- RyggFigGjsnBox(RegData=RegData,  outfile='EQ5DPre.pdf',
			valgtVar='EQ5DPre', ktr=ktr, tittel=0, valgtMaal='', enhetsUtvalg=enhetsUtvalg)
	dum <- RyggFigGjsnBox(RegData=RegData,  outfile='OswPre.pdf', 
			valgtVar='OswTotPre', ktr=ktr, tittel=0, valgtMaal='', enhetsUtvalg=enhetsUtvalg)
	dum <- RyggFigGjsnBox(RegData=RegData,  outfile='SmBePre.pdf', 
			valgtVar='SmBePre', ktr=ktr, tittel=0, valgtMaal='', enhetsUtvalg=enhetsUtvalg)
	dum <- RyggFigGjsnBox(RegData=RegData,  outfile='SmRyPre.pdf', 
			valgtVar='SmRyPre', ktr=ktr, tittel=0, valgtMaal='', enhetsUtvalg=enhetsUtvalg)
@
...
dum <- RyggFigGjsnBox(RegData=RegData,  outfile='OswEndr.pdf', 
		valgtVar='OswEndr', ktr=ktr, tittel=0, valgtMaal='', enhetsUtvalg=enhetsUtvalg)
@
 

\begin{figure}[h] 
\centerline{
  \scalebox{\Sexpr{s1}}{\includegraphics{OswEndr.pdf}}
  }
  \caption{\Sexpr{paste0('Figuren viser gjennomsnittlig endring i Oswestry, ', ktrtxt,  ' etter operasjon, per år.')}}
  \label{fig:OswEndr}
\end{figure}
...
dum <- RyggFigGjsnBox(RegData=RegData,  outfile='EQ5DEndrTid1.pdf', 
		valgtVar='EQ5DEndr', ktr=1, tittel=0, valgtMaal='', enhetsUtvalg=enhetsUtvalg)
dum <- RyggFigGjsnBox(RegData=RegData,  outfile='EQ5DEndrTid2.pdf', 
		valgtVar='EQ5DEndrPre', ktr=2, tittel=0, valgtMaal='', enhetsUtvalg=enhetsUtvalg)
@

\begin{figure}[h] 
\centerline{
  \scalebox{\Sexpr{s2}}{\includegraphics{EQ5DEndrTid1.pdf}}
  \scalebox{\Sexpr{s2}}{\includegraphics{EQ5DEndrTid2.pdf}}
  }
...
dum <- RyggFigGjsnBox(RegData=RegData,  outfile='SmRyggEndrPre.pdf', 
		valgtVar='SmRyggEndrPre', ktr=ktr, tittel=0, valgtMaal='', enhetsUtvalg=enhetsUtvalg)
dum <- RyggFigGjsnBox(RegData=RegData,  outfile='SmRyggEndrTid.pdf', 
		valgtVar='SmRyggEndr', ktr=ktr, tittel=0, valgtMaal='', enhetsUtvalg=enhetsUtvalg)
@

\begin{figure}[h] 
\centerline{
  \scalebox{\Sexpr{s2}}{\includegraphics{SmRyggEndrPre.pdf}}
  \scalebox{\Sexpr{s2}}{\includegraphics{SmRyggEndrTid.pdf}}
  }
...
dum <- RyggFigGjsnBox(RegData=RegData,  outfile='SmBeinEndrPre.pdf', 
		valgtVar='SmBeinEndrPre', ktr=ktr, tittel=0, valgtMaal='', enhetsUtvalg=0)
dum <- RyggFigGjsnBox(RegData=RegData,  outfile='SmBeinEndrTid.pdf', 
		valgtVar='SmBeinEndr', ktr=ktr, tittel=0, valgtMaal='', enhetsUtvalg=0)
@

\begin{figure}[h] 
\centerline{
  \scalebox{\Sexpr{s2}}{\includegraphics{SmBeinEndrPre.pdf}}
  \scalebox{\Sexpr{s2}}{\includegraphics{SmBeinEndrTid.pdf}}
  }
...
<<FigNytte, echo=FALSE>>=
	Nytte <- RyggFigAndelStabelTid(RegData=RegData, outfile='FigNytte.pdf', valgtVar='Nytte', reshID=0,
	hovedkat=hovedkat, tidlOp=tidlOp, ktr=ktr, enhetsUtvalg = enhetsUtvalg, tittel=0)
@

\begin{figure}[h] 
	\begin{center}
  \scalebox{\Sexpr{s1}}{\includegraphics{FigNytte.pdf}}
	\end{center}
...
<<FigFornoyd, echo=FALSE>>=
	Fornoyd <- RyggFigAndelStabelTid(RegData=RegData, outfile='FigFornoyd.pdf', valgtVar='Fornoyd', reshID=0,
		enhetsUtvalg=enhetsUtvalg, hovedkat=hovedkat, tidlOp=tidlOp, ktr=ktr, tittel=0)
@

\begin{figure}[h] 
	\begin{center}
\scalebox{\Sexpr{s1}}{\includegraphics{FigFornoyd.pdf}}
	\end{center}
...
\Huge{ Her starter det som i årsrapporten var etter samlerapporten.}
...
47 \% av de som opereres er kvinner. Gjennomsnittsalderen har økt fra 52 år til 56 år
i perioden. Ryggkirurgi øker mest i den eldste og mest sårbare delen av
...
all hovedsak håndterer denne pasientgruppen. I 2015 ble 22 \% (n=1010) av alle
ryggoperasjonene meldt til NKR utført på personer over 69 år.

<<Figurer1, echo=FALSE, eval=FALSE>>=
Alder70 <- RyggFigAndelTid(RegData=RegData1aar, valgtVar='Alder70', datoFra=datoFra, datoTil=datoTil, 
			enhetsUtvalg=enhetsUtvalg, outfile='Alder70.pdf', tittel=0)
...
sammensatte tilstander. Figur 3.4 viser grad av helseplager før og etter operasjon
for prolaps. Gjennomsnittlig ODI score var 46 før operasjon og 17 ett år etter
operasjon. Dette betyr at funksjonssvikten ble redusert fra alvorlig til minimal for
gjennomsnittspasienten. Pasienter operert for spinal stenose (figur 3.5) fikk også
betydelig bedring (ODI redusert fra 41 til 23), men mange har fortsatt moderat
funksjonssvikt ett år etter kirurgi. De som ble operert med fusjon (figur 3.6) har
...
Danmark. Foreløpige analyser tyder på at resultatene vist nedenfor er de samme i
de tre nordiske landene.
Figur 3.4: Prolapsoperasjon. Pasientrapportert bedring i fysisk funksjon og
livskvalitet.
Figur 3.5: Operasjon for spinal stenose. Pasientrapportert bedring i fysisk funksjon
og livskvalitet (ODI).
Figur 3.6: Avstivningsoperasjon. Pasientrapportert bedring i fysisk funksjon og
livskvalitet (ODI).
...
\tekstbf{Dette er vel et eget avsnitt et annet sted i årsrapporten}
...
kvalitetsindikator.
For NKR degenerativ rygg var dekningsgraden for alle operasjoner utført i Norge 63
\% i 2015. 93 \% av sykehusavdelingene rapporterte data. For NKR degenerativ nakke
var tilsvarende tall 75 og 100 \%. Den relativt lave dekningsgraden skyldes i første
rekke manglende fokus på kvalitetssikring hos ledelsen og dårlige rutiner for
rapportering, spesielt i legegruppen. Størst er problemet for akutt kirurgi, mest i
helger, høytider og ferier.
Innen offentlig helsetjeneste synes kvalitetssikring av egen virksomhet å være høyt
prioritert på Sørvest-landet og i Sør-Trøndelag, men ikke i Helse Sørøst og Møre &
Romsdal HF.
Noen sykehus rapporterte overhodet ikke til NKR i 2015. Dette gjelder Sykehuset
Vestfold (Tønsberg/Larvik), Førde Sjukehus og Kristiansund sjukehus*.
*Har startet i 2016
- Side 14 -
Figur 3.7: Dekningsgrad NKR degenerativ rygg
Figur 3.8: Dekningsgrad NKR degenerativ nakke


\subsection{Prolapskirurgi (alle kategorier)}

Liggetiden på sykehus i forbindelse med operasjon har gått ned med 1/2 døgn i
perioden.

<<Figurer1, echo=FALSE, eval=FALSE>>=
LiggetidProlaps <- RyggFigGjsnBox(RegData=RegData, valgtVar='Liggedogn', datoFra=datoFra, datoTil=datoTil, 
...
Figur 3.9: Gjennomsnittlig liggetid per år for pasienter operert for prolaps.

Andelen som er operert ved hjelp av synsfremmende midler (mikroskop eller
lupebriller), som har åpenbare fordeler, har økt fra 80 \% i 2010 til 95 \% i 2015. NKR
har tidligere vist at multiple reoperasjoner har en minimal og potensiell skadelig
effekt. Andelen som har vært operert mer enn 2 ganger tidligere har gått ned med 1
prosent (fra 5,6 \% i 2010 til 4,5 \% i 2015). For alle typer prolapsoperasjoner har
andel sårinfeksjoner (pasientrapportert) har blitt redusert med 1 \% i perioden mens
bruk av forbyggende antibiotikabehandling har økt med 15 \%. Andelen som har søkt
uføretrygd før operasjon har gått ned fra 13,5 til 11,5 \% i perioden. Andelen
pasienter som er fornøyde med behandlingen de fikk på sykehuset (PREM) har økt
fra 83 til 85 \%.

\subsubsection{Sykehusvise resultater}
\textbf{Her er det vel verd å framheve at det er kvalitetsindikatorer som vises}

Resultater for planlagt, første gangs prolapsoperasjon i perioden 2010-2015 er vist
nedenfor. Kun avdelinger med mer enn 30 registrerte operasjoner i er med i
...
\subsubsection{Effektiv håndtering (rett tid)}
...
operasjonstidspunkt var uendret fra 2010 til 2015 (47\%).
...
<<Figurer1, echo=FALSE, eval=FALSE>>=
LiggetidProlaps <- RyggFigAndelerGrVar(RegData=RegData1aar, valgtVar='SympVarighUtstr', datoFra=datoFra, datoTil=datoTil, 
			hovedkat = 1, outfile='SympVarighUtstr.pdf', tittel=0)
...
\begin{figure}[ht]
	\centering \includegraphics[width= \Sexpr{figKrymp1}\textwidth]{SympVarighUtstr.pdf}
	\caption{\label{fig:SympVarighUtstr} Varighet av bensmerter på operasjonstidspunkt for prolapskirurgi. NB: Har tidligere i kapitlet vist nøyaktig samme figur...}}
\end{figure}

Figur 3.10: Varighet av bensmerter på operasjonstidspunkt for prolapskirurgi.
Vist tidligere nøyaktig samme figur...
...
kirurgi.
Uønsket resultat I.
Pasienter som opplever liten endring av bensmerter (under 1,5 på smerteskalaen)
vil ofte oppfatte totalsituasjonen sin som uendret eller verre etter prolapskirurgi.
Figur 3.13: Andel pasienter som opplever mindre enn 1,5 poeng forbedring på skala
for bensmerter (NRS) ett år etter prolapskirurgi.
- Side 21 -
Uønsket resultat II
...
I. Sårinfeksjon.
Årsakene til sårinfeksjon er komplekse. I 2015 fikk 98-100 \% av pasienter som
...
prolapskirurgi.
- Side 25 -
II. Durarift (rift på ryggmargshinnen)
III.
...
- Liggetiden er fortsatt redusert i 2015. Dette kan knyttes til økt bruk av
